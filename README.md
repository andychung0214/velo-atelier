...
玩家可以從各個角度旋轉、縮放並檢視一輛依真實 700C 公路車比例程式化建立的高細節車體，再依騎乘想像更換車架幾何、碟煞或 C 夾煞、框高或板輪、機械或電子變速、彎把與把手帶。每個主要零件都可點選，介面會說明它在騎乘中的用途。

中文名稱為「疾輪工房」，類型為「3D 公路車訂製模擬遊戲」。視覺採用義式競速海報風格：紙張暖白、賽車紅、森林綠與工業黑，搭配編輯式格線和克制的動態效果。

## 特色

- 程式化 three.js 公路車採真實輪距、輪徑、五通下沉、座管角與頭管角範圍，包含車架、前叉、輪組、煞車、傳動、曲柄、飛輪、座艙、座墊與水壺架。
- 零件網格具有獨立語意角色；頭管、上管、下管、叉腳、煞車、傳動與接觸點會對應正確的用途說明。
...
src/data/               零件目錄與工房預設
src/scene/              公路車幾何基準、three.js 場景、材質、模型與文字貼花
src/ui/                 客製控制台
...
兩個指令目前都會執行 Node.js 內建 `node:test` 測試，涵蓋配置正規化、儲存降級、跨語系文字、公路車幾何比例、3D 零件角色與安裝位置、響應式相機規則、HTML／CSS 結構與靜態伺服器。瀏覽器手動測試請依 [docs/TEST-PLAN.md](docs/TEST-PLAN.md) 執行。
...
- 模型依常見競賽公路車比例重建，但不是特定品牌的 CAD、工程尺寸、零件相容性或空氣力學模擬結果。
